...
function generatePassword(length) {
  ...
  for(i = 0; i <= length; i++){
    let randome = randomInt(0,4)
    let arrayRandome = [];

    if (shouldIncludeLowerCase()){
arrayRandome.push(randomLowerCase())
    }
    if (shouldIncludeNumbers()){
      arrayRandome.push(randomNumber())
    }
    if (shouldIncludeSymbols()){
      arrayRandome.push(randomSymbol())
    }
    if(shouldIncludeUpperCase){
      arrayRandome.push(randomUpperCase())
    }
    pass = pass + arrayRandome[randomInt(0,arrayRandome.length)]
    
    /*if(randome == 0 & shouldIncludeLowerCase())
    {
      pass = pass + randomLowerCase()
    }
    if(randome == 1 & shouldIncludeNumbers())
    {
      pass = pass + randomNumber()
    }
    if(randome == 2 & shouldIncludeSymbols())
    {
      pass = pass +randomSymbol()
    }
    if(randome == 3 & shouldIncludeUpperCase())
    {
